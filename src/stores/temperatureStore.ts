...
export const useTemperatureStore = create<
  TemperatureState & TemperatureActions
>((set, get) => ({
  currentValue: null,
  history: {},
  metadata: [],
  ...
  initialize: async () => {
    try {
      ...
      const temperatureItems = filterItemsBySemanticProperty(
        items,
        PROPERTY_TEMPERATURE
      );
      set({
  ...
  updateValue: (itemName, value, timestamp) => {
    ...
    set((state) => {
      ...
      // Keep only last 2 hours (7200000 ms)
...
(async () => {
  await useTemperatureStore.getState().initialize();
