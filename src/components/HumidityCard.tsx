...
const HumidityCard: React.FC<HumidityCardProps> = ({ location }) => {
  ...
  const getBackgroundTint = (value: number | null) => {
    ...
    if (value < 30) return "bg-gradient-to-t from-blue-500/30 to-transparent";
    if (value <= 60) return "bg-gradient-to-t from-green-500/30 to-transparent";
    ...
    <div className="w-full rounded-2xl p-8 relative overflow-hidden bg-surface/60">
      ...
      <div className="relative z-10">
        <div className="absolute top-2 left-2">
          <MdWaterDrop className="w-8 h-8 text-white/80 drop-shadow-lg" />
        </div>
        <div className="pt-12">
          {loading ? (
            <p>Loading…</p>
